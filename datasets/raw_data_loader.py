...
class CustomRawDataset:
    """
    Custom Dataset for RAW EEG data.

    Attributes:
        config (DataConfig): Configuration object containing dataset parameters.
        paths (Paths): Object containing dataset paths.
        ...
        feature_list (list): List of features to extract from the EEG data.
        num_threads (int): Number of threads to use for data loading.
    """
    ...
    def __init__(
        ...
        if invalid_features:
            ValueError(
                f"Invalid features were given: {invalid_features}. Accepted features are: {FEATURES}"
            )
    ...
    def load_x_y(self):
        ...
        eeg_paths = glob(
            f"{self.paths.TRAIN_EEGS if self.mode=='train' else self.paths.TEST_EEGS}*.parquet"
        )
        ...
        sample_eeg = pd.read_parquet(
            f"{self.paths.TRAIN_EEGS if self.mode=='train' else self.paths.TEST_EEGS}{eeg_ids[0]}.parquet"
        ).iloc[:10]
        self.channels = sample_eeg.columns

        one_hot_df = self.get_one_hot()
        features_per_sample = np.zeros(
            ((len(self.main_df), len(self.extract_features(sample_eeg, one_hot_df))))
        )
        ...
        for i, (eeg_id, eeg) in tqdm(enumerate(eegs)):
            ...
            if True in list((eeg[eeg.columns] == 0).all()):
                ...
                self.logger.info(
                    f"EEG with ID {eeg_id} has no value above zero in some channels(s): {list(bools[bools].index)}"
                )
                ...
                if self.mode == "train":
                    for j, (df_i, subsample) in enumerate(subsamples.iterrows()):
                        subsample_eeg = eeg.iloc[
                            int(subsample["eeg_label_offset_seconds"] * 200) : int(
                                subsample["eeg_label_offset_seconds"] * 200 + (50 * 200)
                            )
                        ]
                        expert_lbls[subsamples_added + j - 1] = np.asarray(
                            subsample[self.label_cols]
                        )
                        subsample_eeg_ids[subsamples_added + j - 1] = subsample[
                            "eeg_sub_id"
                        ]
                        features_per_sample[subsamples_added + j - 1] = (
                            self.extract_features(subsample_eeg, one_hot_df)
                        )
        ...
        if self.mode == "test":
            assert i + 1 == len(
                self.main_df
            ), f"Expected the number of eegs in main_df to be equal to the amount of eegs in {self.paths.TEST_EEGS}"

        self.features_per_sample = features_per_sample  # x
        self.lbl_probabilities = expert_lbls / np.sum(expert_lbls, axis=1)[:, None]  # y
        self.subsample_eeg_ids = subsample_eeg_ids
    ...
    def load_data(self, subset_sample_count: int = 0):
        ...
        try:
            csv_path = (
                self.paths.TRAIN_CSV if self.mode == "train" else self.paths.TEST_CSV
            )
            ...
            if subset_sample_count > 0:
                ...
                if subset_sample_count > unique_patients:
                    self.logger.warning(
                        f"Requested {subset_sample_count} samples, but only {unique_patients} unique patients are available."
                    )
                    subset_sample_count = unique_patients

                # Sample one record from each unique patient
                sampled_df = (
                    main_df.groupby("patient_id")
                ...
                if subset_sample_count < unique_patients:
                    sampled_df = sampled_df.sample(
                        n=subset_sample_count, random_state=42
                    ).reset_index(drop=True)
