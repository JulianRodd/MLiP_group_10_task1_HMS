...
def majority_confusion(y_pred, y_true, labels):
    ...
    plot_confusion_matrix(
        cnf_matrix, classes=labels, normalize=False, title="Confusion matrix"
    )
...
def plot_avg_mispred(differences: np.ndarray, by_class=True):
    ...
    if by_class:
        _, axes = plt.subplots(1, len(Generics.LABEL_COLS))
        axes = axes.flatten()
...
def failure_analysis(
    ...
    cnf_mtrx = majority_confusion(
        y_pred=y_pred, y_true=y_true, labels=Generics.LABEL_COLS
    )
...
def run_failure_analysis(
    ...
    data_loader_config: BaseDataConfig,
    model: CustomModel,
):

    model_dirs = [
        f"{Paths.BEST_MODEL_CHECKPOINTS}/best_{model_config.MODEL}_{model_config.NAME}_{data_loader_config.NAME}_fold_{fold}.pth"
        for fold in range(model_config.FOLDS)
    ]
    dataset = CustomDataset(config=data_loader_config, mode="val")
