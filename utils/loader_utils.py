...
from utils.eeg_processing_utils import (
    generate_eeg_from_parquet,
...
def load_main_dfs(data_loader_config, train_val_split=(0.8, 0.2)) -> pd.DataFrame:
    try:
        ...
        prepared_train_df = prepared_train_df[
            ~prepared_train_df["eeg_id"].isin(Generics.OPT_OUT_EEG_ID)
        ]
        ...
        if data_loader_config.ONE_SAMPLE:
            sampled_train_csv_pd = (
                prepared_train_df.groupby("patient_id")
                .sample(n=1, random_state=42)
                .reset_index(drop=True)
            )
            samples_test_csv_pd = (
                test_csv_pd.groupby("patient_id")
                .sample(n=1, random_state=42)
                .reset_index(drop=True)
            )
        else:
        ...
        sampled_train_csv_pd = sampled_train_csv_pd.sample(
            n=train_sample_count, random_state=42
        ).reset_index(drop=True)

        gss = GroupShuffleSplit(
            n_splits=2, train_size=train_val_split[0], random_state=42
        )
        ...
        if data_loader_config.FILTER_BY_AGREEMENT:
            train_df = filter_by_agreement(
                train_df, data_loader_config.FILTER_BY_AGREEMENT_MIN
            )
        if data_loader_config.FILTER_BY_AGREEMENT_ON_VAL:
            val_df = filter_by_agreement(
                val_df, data_loader_config.FILTER_BY_AGREEMENT_MIN
            )

        if data_loader_config.FILTER_BY_ANNOTATOR:
            train_df = filter_by_annotators(
                train_df,
                data_loader_config.FILTER_BY_ANNOTATOR_MIN,
                data_loader_config.FILTER_BY_ANNOTATOR_MAX,
                n_annot=train_df["n_annot"],
            )
        if data_loader_config.FILTER_BY_ANNOTATOR_ON_VAL:
            val_df = filter_by_annotators(
                val_df,
                data_loader_config.FILTER_BY_ANNOTATOR_MIN,
                data_loader_config.FILTER_BY_ANNOTATOR_MAX,
                n_annot=val_df["n_annot"],
            )
...
def load_eeg_data(main_df: pd.DataFrame, mode: str) -> Dict[int, pd.DataFrame]:
    ...
    try:
        ...
        paths_eegs = [
            f
            for f in glob(f"{csv_path}*.parquet")
            if int(os.path.basename(f).split(".")[0]) in eeg_ids
        ]
...
def process_eeg_data(
    ...
    if custom_config is not None:
        eeg_spectrogram = generate_custom_spectrogram_from_eeg(
            eeg_data, feats, custom_config
        )
    else:
...
def load_eeg_spectrograms(
    ...
    try:
        ...
        for eeg_id, eeg_data in tqdm(eeg_data_dict.items(), desc="Processing EEG Data"):
            eeg_id = int(eeg_id)
            eeg_spectrogram = process_eeg_data(
                eeg_data,
                feats,
                use_wavelet,
                mspca_on_raw_eeg,
        ...
        if custom_config is not None:
            if custom_config.get("save", False):
                ...
                config_str = "_".join(
                    [f"{key}-{value}" for key, value in sorted(custom_config.items())]
                )
                np.save(
                    f"{Paths.PRE_LOADED_CUSTOM_EEGS_DIR}eeg_specs_custom_{config_str}.npy",
                    eeg_spectrograms,
                    allow_pickle=True,
                )
...
def normalize_eeg_spectrograms(
    eeg_spectrograms: Dict[int, np.ndarray], normalize_indiv=False
) -> Dict[int, np.ndarray]:
    """
    ...
    if normalize_indiv:
        ...
        eeg_data = np.array(
            [eeg_spectrograms[eeg_id] for eeg_id in eeg_spectrograms.keys()]
        )
...
def load_preloaded_spectrograms(main_df: pd.DataFrame):
    pre_loaded_spectrograms = np.load(
        Paths.PRE_LOADED_SPECTROGRAMS, allow_pickle=True
    ).item()
    return {
        k: v
        for k, v in pre_loaded_spectrograms.items()
        if k in main_df["spectrogram_id"].values
    }
...
def load_spectrograms(main_df: pd.DataFrame, mode: str) -> Dict[int, np.ndarray]:
    ...
    try:
        ...
        paths_spectrograms = [
            f
            for f in glob(
                Paths.TEST_SPECTROGRAMS + "*.parquet"
                if mode == "test"
                else Paths.TRAIN_SPECTROGRAMS + "*.parquet"
            )
