    ...
    try:
        ...
        for k in range(4):
            ...
            for kk in range(4):
                ...
                if DataConfig.USE_WAVELET:
                    x = denoise(x, wavelet=DataConfig.USE_WAVELET)  # denoise function should be imported or defined
...
def _display_eeg_and_spectrogram(img: np.ndarray, signals: list, display_eeg_id: int):
    """
    ...
    for k in range(4):
        ...
        plt.title(f"EEG {display_eeg_id} - Spectrogram {DataConfig.NAMES[k]}")
    plt.show()
    ...
    for k in range(4):
        ...
        plt.plot(range(10_000), signals[k] + offset, label=DataConfig.NAMES[3 - k])
